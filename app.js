...
function isCorrectAnswer(value, expected) {
  ...
  const score = similarityScore(normalizedValue, normalizedExpected);
  return score >= 0.7;
}
...
function showSuccessMessage(stage) {
  ...
  feedbackText.textContent = `Bravo ! C’était bien ${answer}.`;
...
function startConfetti() {
  ...
  for (let index = 0; index < 22; index += 1) {
    ...
    piece.style.background = index % 2 === 0 ? "#f4a593" : "#e8764f";
...
function submitAnswer() {
  ...
  feedbackText.textContent = percent > 45 ? `Presque ! Similarité ${percent} %` : `Ce n'est pas encore ça. (${percent} %)`;
  feedbackText.style.color = "#8b6f72";
}

function requestHelp() {
  if (solvedStages[currentIndex] || currentHelpLevel >= maxHelpLevel) {
    return;
  }
  currentHelpLevel += 1;
